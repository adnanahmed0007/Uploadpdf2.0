        ...
        <div className="flex justify-between items-center h-20">
          ...
          <Link to="/" className="flex items-center gap-3 group transition-transform hover:scale-105 duration-300">
            ...
            <div className="hidden md:block">
              ...
              <p className="text-xs text-gray-600 font-medium">Digital Archive Portal</p>
            </div>
          </Link>
          ...
          <nav className="hidden lg:flex items-center gap-2">
            ...
            <div className="relative">
              <button
                ...
                className={`px-4 py-2.5 rounded-xl font-semibold transition-all duration-200 flex items-center gap-2 ${['/branchdeaprtemntyear', '/userdepartmentpdf', '/notessubjectwise', '/getallview'].includes(location.pathname)
              ...
              <div
                onMouseEnter={() => setIsBrowseOpen(true)}
                onMouseLeave={() => setIsBrowseOpen(false)}
                className={`absolute top-full left-0 mt-2 w-72 bg-white/95 backdrop-blur-xl rounded-2xl shadow-2xl border border-gray-200/50 overflow-hidden transition-all duration-300 ${isBrowseOpen ? 'opacity-100 visible translate-y-0' : 'opacity-0 invisible -translate-y-2'
                  }`}
              >
...
const NavLink = ({ to, label, icon, isActive }) => (
  <Link
    to={to}
    className={`px-4 py-2.5 rounded-xl font-semibold transition-all duration-200 flex items-center gap-2 ${isActive
...
const DropdownLink = ({ to, label, icon, description, isActive }) => (
  <Link
    to={to}
    className={`flex items-start gap-3 px-4 py-3 rounded-xl transition-all duration-200 group ${isActive
...
const MobileNavLink = ({ to, label, icon, onClick, isPrimary, isActive }) => (
  <Link
    to={to}
    onClick={onClick}
    className={`flex items-center gap-3 px-4 py-3 rounded-xl font-semibold transition-all duration-200 ${isPrimary
